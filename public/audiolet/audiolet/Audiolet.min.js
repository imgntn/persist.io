 ...
 * @depends Tuning.js
 */
var EqualTemperamentTuning=function(b){var c=[];for(var a=0;a<b;a++){c.push(a)}Tuning.call(this,c,2)};extend(EqualTemperamentTuning,Tuning);(function(f){function e(m,l,k,j){var g=e.sinks,h;for(h in g){if(g.hasOwnProperty(h)&&g[h].enabled){try{return new g[h](m,l,k,j)}catch(n){}}}throw"No audio sink available."}function d(g){this.boundTo=g;this.buffers=[];g.activeRecordings.push(this)}d.prototype={add:function(g){this.buffers.push(g)},clear:function(){this.buffers=[]},stop:function(){var g=this.boundTo.activeRecordings,h;for(h=0;h<g.length;h++){if(g[h]===this){g.splice(h--,1)}}},join:function(){var m=0,o=0,j=this.buffers,g,p,k,h=j.length;for(k=0;k<h;k++){m+=j[k].length}g=new Float32Array(m);for(k=0;k<h;k++){for(p=0;p<j[k].length;p++){g[o+p]=j[k][p]}o+=j[k].length}return g}};function c(){}e.SinkClass=c;c.prototype={sampleRate:44100,channelCount:2,preBufferSize:4096,writePosition:0,writeMode:"async",channelMode:"interleaved",previousHit:0,ringBuffer:null,ringOffset:0,start:function(k,j,h,g){this.channelCount=isNaN(j)?this.channelCount:j;this.preBufferSize=isNaN(h)?this.preBufferSize:h;this.sampleRate=isNaN(g)?this.sampleRate:g;this.readFn=k;this.activeRecordings=[];this.previousHit=+new Date;this.asyncBuffers=[];this.syncBuffers=[]},process:function(h,j){this.ringBuffer&&(this.channelMode==="interleaved"?this.ringSpin:this.ringSpinInterleaved).apply(this,arguments);this.writeBuffersSync.apply(this,arguments);if(this.readFn){if(this.channelMode==="interleaved"){this.readFn.apply(this,arguments)}else{var g=e.deinterleave(h,this.channelCount);this.readFn.apply(this,[g].concat([].slice.call(arguments,1)));e.interleave(g,this.channelCount,h)}}this.writeBuffersAsync.apply(this,arguments);this.recordData.apply(this,arguments);this.previousHit=+new Date;this.writePosition+=h.length/j},record:function(){return new d(this)},recordData:function(h){var k=this.activeRecordings,j,g=k.length;for(j=0;j<g;j++){k[j].add(h)}},writeBuffersAsync:function(j){var h=this.asyncBuffers,g=j.length,k,o,m,q,p;if(h){for(m=0;m<h.length;m++){k=h[m];o=k.b.length;p=k.d;k.d-=Math.min(p,g);for(q=0;q+p<g&&q<o;q++){j[q+p]+=k.b[q]}k.b=k.b.subarray(q+p);m>=o&&h.splice(m--,1)}}},writeBuffersSync:function(j){var h=this.syncBuffers,g=j.length,k=0,m=0;for(;k<g&&h.length;k++){j[k]+=h[0][m];if(h[0].length<=m){h.splice(0,1);m=0;continue}m++}if(h.length){h[0]=h[0].subarray(m)}},writeBufferAsync:function(h,j){h=this.mode==="deinterleaved"?e.interleave(h,this.channelCount):h;var g=this.asyncBuffers;g.push({b:h,d:isNaN(j)?~~((+new Date-this.previousHit)/1000*this.sampleRate):j});return g.length},writeBufferSync:function(h){h=this.mode==="deinterleaved"?e.interleave(h,this.channelCount):h;var g=this.syncBuffers;g.push(h);return g.length},writeBuffer:function(){this[this.writeMode==="async"?"writeBufferAsync":"writeBufferSync"].apply(this,arguments)},getSyncWriteOffset:function(){var g=this.syncBuffers,j=0,h;for(h=0;h<g.length;h++){j+=g[h].length}return j},getPlaybackTime:function(){return this.writePosition-this.preBufferSize},ringSpin:function(j){var k=this.ringBuffer,h=j.length,g=k.length,o=this.ringOffset,n;for(n=0;n<h;n++){j[n]+=k[o];o=(o+1)%g}this.ringOffset=o},ringSpinDeinterleaved:function(q){var p=this.ringBuffer,o=q.length,g=p.length,k=p[0].length,s=g*k,j=this.ringOffset,r,h;for(r=0;r<o;r+=g){for(h=0;h<g;h++){q[r+h]+=p[h][j]}j=(j+1)%k}this.ringOffset=h}};function a(k,j,g,h){g=g||j.prototype;j.prototype=new e.SinkClass();j.prototype.type=k;j.enabled=!h;for(h in g){if(g.hasOwnProperty(h)){j.prototype[h]=g[h]}}a[k]=j}a("moz",function(){var q=this,j=0,p=null,o=new Audio(),n,m,k,l,g;q.start.apply(q,arguments);q.preBufferSize=isNaN(arguments[2])?q.sampleRate/2:q.preBufferSize;function h(){if(p){n=o.mozWriteAudio(p);j+=n;if(n<p.length){p=p.subarray(n);return p}p=null}m=o.mozCurrentSampleOffset();k=Number(m+q.preBufferSize*q.channelCount-j);if(k>0){l=new Float32Array(k);q.process(l,q.channelCount);n=o.mozWriteAudio(l);if(n<l.length){p=l.subarray(n)}j+=n}}o.mozSetup(q.channelCount,q.sampleRate);q.kill=e.doInterval(h,20);q._bufferFill=h;q._audio=o},{getPlaybackTime:function(){return this._audio.mozCurrentSampleOffset()/this.channelCount}});var b=[];a("webkit",function(o,m,k,j){var g=this,l=new (window.AudioContext||webkitAudioContext)(),n=l.createJavaScriptNode(k,0,m);g.start.apply(g,arguments);function h(w){var p=w.outputBuffer,s=p.numberOfChannels,u,r,t=p.length,x=p.size,v=new Array(s),q=new Float32Array(t*s);for(u=0;u<s;u++){v[u]=p.getChannelData(u)}g.process(q,g.channelCount);for(u=0;u<t;u++){for(r=0;r<s;r++){v[r][u]=q[u*g.channelCount+r]}}}n.onaudioprocess=h;n.connect(l.destination);g.sampleRate=l.sampleRate;g._context=l;g._node=n;g._callback=h;b.push(n)},{kill:function(){},getPlaybackTime:function(){return this._context.currentTime*this.sampleRate},});a.webkit.fix82795=b;a("dummy",function(){var g=this;g.start.apply(g,arguments);function h(){var j=new Float32Array(g.preBufferSize*g.channelCount);g.process(j,g.channelCount)}g.kill=e.doInterval(h,g.preBufferSize/g.sampleRate*1000);g._callback=h},null,true);e.sinks=e.devices=a;e.Recording=d;e.doInterval=function(n,j){var h=typeof window==="undefined"?undefined:window.MozBlobBuilder||window.WebKitBlobBuilder||window.MSBlobBuilder||window.OBlobBuilder||window.BlobBuilder,m,l,g;if((e.doInterval.backgroundWork||e.devices.moz.backgroundWork)&&h){try{g=new h();g.append('setInterval(function(){ postMessage("tic"); }, '+j+");");l=(window.MozURL||window.webkitURL||window.MSURL||window.OURL||window.URL).createObjectURL(g.getBlob());m=new Worker(l);m.onmessage=function(){n()};return function(){m.terminate();(window.MozURL||window.webkitURL||window.MSURL||window.OURL||window.URL).revokeObjectURL(l)}}catch(k){}}m=setInterval(n,j);return function(){clearInterval(m)}};e.doInterval.backgroundWork=true;(function(){function g(h,j){if(h&&j){g[h]=j}else{if(h&&g[h] instanceof Function){e.interpolate=g[h]}}return g[h]}e.interpolation=g;g("linear",function(h,m){var l=Math.floor(m),k=l+1,j=m-l;k=k<h.length?k:0;return h[l]*(1-j)+h[k]*j});g("nearest",function(h,j){return j>=h.length-0.5?h[0]:h[Math.round(j)]});g("linear")}());e.resample=function(q,g,t,u,j){var p=arguments.length,m=p===2?g:p===3?g/t:u/g*j/t,o=q.length,h=Math.ceil(o/m),s=new Float32Array(h),r,k;for(r=0,k=0;r<o;r+=m){s[k++]=e.interpolate(q,r)}return s};e.deinterleave=function(h,o){var g=h.length,m=g/o,j=[],k,p;for(k=0;k<o;k++){j[k]=new Float32Array(m);for(p=0;p<m;p++){j[k][p]=h[p*o+k]}}return j};e.interleave=function(j,o,h){o=o||j.length;var g=j[0].length,m=j.length,k,p;h=h||new Float32Array(g*o);for(k=0;k<m;k++){for(p=0;p<g;p++){h[k+p*o]=j[k][p]}}return h};e.mix=function(j){var h=[].slice.call(arguments,1),g,k,m;for(m=0;m<h.length;m++){g=Math.max(j.length,h[m].length);for(k=0;k<g;k++){j[k]+=h[m][k]}}return j};e.resetBuffer=function(h){var g=h.length,j;for(j=0;j<g;j++){h[j]=0}return h};e.clone=function(j,g){var h=j.length,k;g=g||new Float32Array(h);for(k=0;k<h;k++){g[k]=j[k]}return g};e.createDeinterleaved=function(k,j){var g=new Array(j),h;for(h=0;h<j;h++){g[h]=new Float32Array(k)}return g};f.Sink=e}(function(){return this}()));