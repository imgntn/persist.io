...
function scene2Loader() {
	...
	*/
	...
	txtparent.position.z = 500;
	txtparent.position.x = 0;
...
function startScene2Tweens() {
	setInterval(changePlane, 500);
	new TWEEN.Tween(planesContainer.position).to({
		z: -625
	}, 100000).start();
		new TWEEN.Tween(planesContainer.position).to({
		y:2000
	}, 100000).start();
	new TWEEN.Tween(planesContainer.scale).to({
		x: 3
	}, 100000).start();
	new TWEEN.Tween(planesContainer.scale).to({
		x: 3
